Fodderly read me

Known issues:
-- Uncaught SyntaxError: Unexpected token u on "Sign Up" (doesn't effect functionality)
-- User credentials wrong currently yields JSON
-- dry up recipe show code
-- scrolling need to move mouse before it registers where it used to be
-- collections drop down in recipe show also includes those it is not already added to

Wednesday to dos:
-- fix user credentials
-- post to fodderly.com

To dos:
-- bug such that can only click on List once
-- check and fix when get slow updates
-- need to add Notes
-- user can add recipes
-- add infinite scroll

Nice to haves:
-- user can export recipe as PDF

